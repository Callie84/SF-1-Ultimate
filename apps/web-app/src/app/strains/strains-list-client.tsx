...
import { useStrains, type StrainsResponse } from '@/hooks/use-strains';
...
export function StrainsListClient({
  initialStrains,
}: {
  initialStrains?: StrainsResponse;
}) {
  ...
  // Server-Initialdaten nur nutzen, solange die Query der ersten (server-geholten)
  // Seite entspricht — also unverändertes Filter-/Such-/Seiten-Set.
  const isInitialQuery = page === 1 && !search && type === 'all';

  const { data, isLoading } = useStrains(
    options,
    isInitialQuery ? initialStrains : undefined
  );
