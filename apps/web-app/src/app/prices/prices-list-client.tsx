...
import {
  Search, TrendingDown, ExternalLink, Filter,
...
export interface SeedWithPrices {
  _id: string;
  name: string;
  slug: string;
  breeder: string;
...
export interface PricesBrowseResponse {
  seeds: SeedWithPrices[];
  total: number;
  breeders?: string[];
}
...
export function PricesListClient({
  initialData,
}: {
  initialData?: PricesBrowseResponse;
}) {
  const [query, setQuery] = useState('');
  const [seeds, setSeeds] = useState<SeedWithPrices[]>(initialData?.seeds || []);
  const [total, setTotal] = useState(initialData?.total || 0);
  const [breeders, setBreeders] = useState<string[]>(initialData?.breeders || []);
  ...
  // Erster Fetch überspringen, wenn Server-Initialdaten vorliegen (die
  // entsprechen exakt der Default-Browse-Query: page 0, keine Filter, sort=price).
