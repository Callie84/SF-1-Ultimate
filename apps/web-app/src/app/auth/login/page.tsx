...
export default function LoginPage() {
  ...
  const { login, user, isLoading: authLoading, refreshUser } = useAuth();
  const router = useRouter();

  // redirect-Parameter auslesen (window.location statt useSearchParams um Prerender-Fehler zu vermeiden)
  useEffect(() => {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      // 2FA required → redirect to 2FA page with mfa_token
      if (res.requires2FA && res.mfa_token) {
        router.push(`/auth/2fa?token=${res.mfa_token}&redirect=${encodeURIComponent(redirectTo)}`);
        return;
      }
      ...
      toast.error(
        error.response?.data?.error || 'Login fehlgeschlagen. Bitte überprüfe deine Zugangsdaten.'
      );
    } finally {
      setIsLoading(false);
    }
