...
## 2026-06-30 — CI/CD-Pipeline (tag-basierter Deploy) + Auth-Incident behoben
**Sicherheit:** origin auf SSH umgestellt (read-only Deploy-Key `/root/.ssh/sf1_repo_deploy`), Server-Token entfällt. Claude Code vom Server entfernt (`/root/.claude` gelöscht, Prozesse gekillt). 4 GitHub-Secrets gesetzt (DEPLOY_SSH_KEY, SERVER_HOST/USER/PATH), CI-Key (Key A) in authorized_keys.
...
## 2026-07-05 (Abend) — Docker Build Check: falsche Dockerfile-Referenz fuer auth-service
**Fehlerbild:** `npm run build` (tsc) schlug im Docker-Build fehl mit 7 Fehlern in 4 Dateien: `Module "@prisma/client" has no exported member 'User'/'UserRole'` (jwt.service.ts, token.service.ts, user.service.ts) + 3x "implicitly has an 'any' type" (analytics.routes.ts x2, auth.routes.ts:1453).

**Ursachen-Analyse:** ALLE 7 Fehler hatten eine einzige gemeinsame Wurzel, kein einziger echter Code-Fehler. `.github/workflows/docker-build.yml` referenzierte fuer `auth-service` die GENERISCHE `docker/Dockerfile.production` — diese hat KEINEN `npx prisma generate`-Schritt, weil sie fuer alle MongoDB-Services gedacht ist (die kein Prisma nutzen). Dadurch bleibt `@prisma/client` beim Docker-Build ungeneriert (nur Stub-Typen), `User`/`UserRole` fehlen komplett — und das reisst automatisch auch scheinbar unabhaengige Stellen mit (z.B. `user.totpBackupCodes.findIndex(h => ...)` wird zu `any`, weil `user` selbst schon `any`-artig kollabiert). `apps/auth-service/Dockerfile` (die service-eigene, korrekte Datei) hat dagegen einen dedizierten Prisma-Stage mit `RUN npx prisma generate` VOR dem TypeScript-Build.
...
## 2026-07-17 -- ToS-4.2a-Compliance-Fix im research-service (geplant + umgesetzt + deployed + live verifiziert)

**Kontext:** Beim vorherigen Exa-Pilot wurde entdeckt, dass `POST /partners` (research-service) rohe Exa-Suchergebnisse 1:1 per `res.json` an den Aufrufer zurueckgibt -- Verstoss gegen Exa ToS 4.2a (Rohdaten duerfen nie an Endnutzer, nur in interne MongoDB-Collections). Dieser Eintrag dokumentiert Planung, Umsetzung, Deploy und Live-Verifikation in einer durchgehenden Session (Claude Chat fuer Planung, Claude Code fuer Ausfuehrung).

**Planung (Claude Chat):** Vollstaendiger Diff-Plan vor jeder Aenderung erstellt und Callie gezeigt (Datei `2026-07-17-plan-tos-4.2a-fix-research-service.md`). Zwei offene Design-Fragen mit Vor-/Nachteilen bewusst an Callie zurueckgegeben statt selbst zu entscheiden: (A) ob ein interner `GET /partners/:id/raw`-Endpoint existieren soll (Callie: ja, Vorschlag A bestaetigt), (B) Verhalten bei MongoDB-Ausfall beim Start (Callie: ok, "startet trotzdem" statt Prozessabbruch, passend zum bestehenden Stil des Service bei fehlendem Exa-Key).

**Umsetzung (Lenovo, Claude Chat per Filesystem-MCP):**
1. `apps/research-service/src/config/mongodb.ts` (neu) -- Mongo-Connect nach Muster von tools-service, aber non-blocking bei Fehler.
2. `apps/research-service/src/models/PartnerCandidate.model.ts` (neu) -- Collection `partner_candidates`, Felder `query`/`category`/`rawResult`/`resultCount`/`status` (new/reviewed/contacted/rejected).
3. `apps/research-service/src/routes/partners.ts` ersetzt -- `POST /partners` schreibt Rohergebnis nach Mongo, gibt nur gefiltertes DTO zurueck (`candidateId`, `resultCount`, `items[]` mit title/url/publishedDate, kein `text`/Score). Neue Routen `GET /partners` (Liste ohne rawResult) und `GET /partners/:id/raw` (nur intern, kein Traefik-Routing).
4. `apps/research-service/src/index.ts` -- Mongo-Connect vor `app.listen`, aber Service startet auch bei Verbindungsfehler weiter (nur Logging).
5. `apps/research-service/package.json` -- `mongoose: ^8.0.0` ergaenzt. Bewusst NICHT `@types/mongoose` ergaenzt (laut Typschulden-Analyse vom 2026-07-06 ein deprecated Stub-Paket, Mongoose 8 bringt eigene Typen mit).
6. `docker-compose.yml` -- `MONGODB_URL` fuer research-service ergaenzt (neue logische DB `sf1_research` auf dem bestehenden Mongo-Container, kein neues Secret noetig) + `depends_on.mongodb.condition: service_healthy`.

**Ausfuehrung/Deploy (Claude Code):** `npm install --legacy-peer-deps` im research-service, `tsc --noEmit` gruen, lokaler Docker-Test erfolgreich (Response ohne Rohdaten verifiziert, rawResult nur in Mongo). Danach Deploy-Reihenfolge eingehalten: Lenovo committet -> GitHub gepusht (Commit `aa57072`) -> Server per `git pull --ff-only` (HEAD jetzt `aa57072`) -> research-service auf dem Server neu gebaut und gestartet.

**Live-Verifikation auf dem Server:** Auf Wunsch von Callie zusaetzlich ein echter Test gegen den produktiven Exa-Zugang gefahren, nachdem `EXA_API_KEY` dort gesetzt wurde (Modus jetzt **active**, Monatslimit 900). `POST /partners` mit `{"query":"cannabis seedbank Deutschland"}` (1 Exa-Request verbraucht, 899 uebrig) lieferte nachweislich nur `candidateId`/`resultCount`/`items[]` zurueck. Gegenprobe direkt in MongoDB (`sf1_research.partner_candidates`): 1 Dokument, Status `new`, `rawResult` vollstaendig inkl. Volltext (`results[0].text` = 1.532 Zeichen) -- der Fix greift damit nachweislich auch produktiv, nicht nur lokal.

**Regel-22a-Selbstpruefung -- was unerwartet war:**
1. **research-service lief vor diesem Deploy noch nie auf dem Server** (Erststart, kein vorheriger Container vorhanden) -- war implizit als "laeuft schon" angenommen worden, bis der Deploy es zeigte. Kein Fehler, aber unerwartet.
2. **Beim GitHub-Push wurden 55 offene Dependabot-Findings auf `main` sichtbar** (1 critical, 23 high, 24 moderate, 7 low) -- durch den Compliance-Commit selbst nicht verursacht (lokales `npm install` im research-service: 0 Vulnerabilities), aber ein bisher nicht sichtbares Risiko, das erst durch diese Aktion auffiel. In SESSION-START-TODO.md als eigener Punkt mit hoher Prioritaet aufgenommen.
3. **Ein automatisierter Wochenbericht (10.-17.07., derselbe Eintrag wie oben in dieser Datei) lag beim Server-Pull uncommittet in `DOKUMENTATION.md` auf dem Server** und musste per `git stash` -> `pull` -> `stash pop` erhalten werden, bevor der Fast-Forward sauber durchlaufen konnte. **Das ist bereits das zweite Mal**, dass ein automatisierter Schreibprozess auf dem Server etwas in `DOKUMENTATION.md` hinterlaesst, ohne es zu committen (vgl. fruehere Learnings zu unversionierten Server-Skripten) -- moeglicherweise derselbe taegliche Mastertest-Cron oder ein verwandter Prozess. Ursache noch nicht abschliessend geklaert, Diff liegt weiterhin uncommittet auf dem Server (SESSION-START-TODO.md Punkt 10).
4. **Claude Code hat bei der MongoDB-Gegenprobe auf dem Server zunaechst einen eigenen Fehler gemacht** (`findOne()` mit einem `sort`-Objekt an Stelle der Projection uebergeben, dadurch kamen leere Felder zurueck), dies aber selbst bemerkt und die Abfrage sauber wiederholt, bevor das Ergebnis als Beweis gemeldet wurde -- gutes Beispiel fuer Selbstkorrektur vor der "fertig"-Meldung, aber auch ein Hinweis, Verifikations-Queries generell auf Plausibilitaet zu pruefen statt sie blind als Beweis zu nehmen.
5. **Beim Nachtragen dieser Session in `SESSION-START-TODO.md` (Claude Chat, Filesystem-MCP) traten zwei kleinere technische Stoerungen auf:** ein `edit_file`-Aufruf produzierte zweimal denselben Encodingfehler beim Wort "regulaer" (Umlaut + Folgebuchstaben gingen verloren, wurde zu "regaer"), einmal zusaetzlich mitten im Satz abgeschnitten. Beide Faelle wurden durch Nachlesen des Diffs bemerkt und sofort korrigiert. Zusaetzlich fror die Filesystem-MCP-Verbindung fuer ca. 4 Minuten ein (bekanntes Muster, siehe Tools-Abschnitt in SF-Brain) und loeste sich von selbst wieder. Lernwert: nach `edit_file`-Aufrufen mit Umlauten/laengeren Ersetzungen den zurueckgegebenen Diff aktiv gegenlesen, nicht nur auf "success" vertrauen.

**Status:** Fix deployed, verifiziert, Exa auf dem Server aktiv. SESSION-START-TODO.md, Kontext-Snapshot.md und Status & Roadmap.md aktualisiert. Nichts mehr offen aus dieser Teilaufgabe -- offene Anschlusspunkte (Dependabot-Audit, Wochenbericht-Diff, moeglicher Review-Flow fuer Partner-Kandidaten) sind als eigene TODO-Punkte erfasst.
